...
- **Instruct, don't justify.** Skills say what to do. Reasoning, alternatives, and history belong in the decision record (`docs/specs/`). If a rule needs defending, defend it in the spec and cite it; keep at most a short clause of reason in the skill, and only where it stops the agent doing the wrong thing.
