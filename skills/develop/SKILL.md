...
2. **Open the governing ADR via the row's `ADR` pointer — read only what specs this sub-task.** A single ADR file → read it. An umbrella `index.md` → read the index (decision + child list), then open **the child ADR(s) this sub-task touches** (usually one; a second only if it genuinely spans two). It's the spec; proceed. Only if the row has **no** pointer *and* no ADR is linked, do a **targeted** look for one matching this feature's scope in its `docs/adr/<workspace>/` — never a blanket read of every ADR.
