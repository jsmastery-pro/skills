...
**Project preference — ask, don't pre-decide, on the core design inputs.** For the **data model**, the **stack**, and the **tool/provider choice**, this engineer wants to be **asked and to pick**, not handed a finished answer to approve. So those move from RECOMMEND to **ASK-open**: present the real, current options as choices, but don't lead with a pre-selected pick or a pre-filled model. Reserve RECOMMEND for small internal implementation details they don't want to weigh in on, and even then offer it as a visible choice. The failure to avoid: dumping a complete data model or stack in a box and asking only "accept or change."
...
The design is an **ordered sequence of stages**, each ending in a **GATE**. For every part the engineer owns (the requirements, the data model, the stack and tool choices, the methods, the business rules) you **ASK, you do not guess**: put real questions to the engineer, build the spec up **from their answers**, and only then SHOW the assembled result back for a sign-off. **Do NOT open a stage with a finished, pre-filled model or a pre-picked stack for them to accept or reject** — that turns designing into rubber-stamping, and it is exactly what the engineer flagged as wrong ("it just gives everything in a box"). The **data model** and the **stack/tool choice** especially are **elicited question by question**, never handed over as a complete guess. **No vital stage is silently decided or skipped.** What you build together, stage by stage, becomes the ADR's `## Requirements` (the acceptance-criteria spine) and `## Build plan`. **Generate every question from *this* feature** — never a fixed list; an auth feature and a reviews feature share none.

**Two kinds of panel — ask open for design inputs, recommend for process gates.**
- **Design-input questions** (what the engineer owns: entities, fields, relationships, rules, which stack layer, which methods) → **ask open.** Offer 2 to 4 real options to make answering fast, plus a free-text **"Other"**, but **do NOT pre-mark one as "(recommended)" and do NOT pre-fill the answer.** The engineer supplies the substance; you are eliciting, not steering. This is the project preference: the engineer would rather be asked than handed a guess to approve.
- **Process / gate panels** (confirm the assembled data model, accept the ADR, the References consent, how to treat an overlapping ADR) → keep **exactly ONE option marked "(recommended)"** with a one-line why, plus **"Other"**. Here a default is helpful because the substance is already on the table and you are just confirming it.
- **Capability-first (both kinds):** use your agent's interactive picker (`AskUserQuestion` on Claude Code); where the agent has no picker, degrade to the **same options as plain text**.

**Still infer the framing, but ASK the design inputs.** Framing (mode, platform, the stack *already in the repo*, constraints) is inferred as above — don't interrogate what you can read. The stages then spend the budget **asking** what the engineer owns: the requirements, the data model, the rules, the scope, and the stack/tool choices for anything not already settled. Within each stage, sort every dimension **INFER / ASK / RECOMMEND** (see *Asks vs acts* and the project preference there): **INFER** silently from the prompt/codebase/`AGENTS.md`; **ASK-open** the engineer for the design inputs (data model, stack, provider, methods, rules) with real options but no pre-marked pick; **RECOMMEND** only the small internal implementation details they don't want to weigh in on. The gate confirms the assembled stage at once.
...
**Stage (c) — Stack & tool selection (ASK the engineer, drill down level by level).** Drill **category → specific option → config**, one question per open level — e.g. *backend shape?* → (a platform/BaaS → which one → which of its features) **or** (custom → database → data-access layer → auth approach → hosting). **Ask the engineer which they want at each level.** Present the **current, real options** for that level as choices so they can pick fast, but **do NOT pre-pick one as the decision** — the choice is theirs (this is the ask-open rule; the stack is a design input the engineer owns). **Generate the options FRESH and CURRENT at runtime — never a hardcoded/canned list** (this category rots fastest); be honest about staleness (*"as of my knowledge; this space moves fast, verify current"*). **Skip any level the existing stack already settles** (INFER from `AGENTS.md` — don't re-ask a decided layer, and prefer reuse of what the project already runs when you list options); drill only where a real choice is open (for an ENHANCEMENT most of this is inferred; for ARCHITECTURE/greenfield it's the core of the conversation). If the engineer explicitly asks for your opinion on a level, give it as **one input they can take or leave**, not as a pre-selected answer. For a **greenfield/foundational (ARCHITECTURE) stack decision**, checking the current tool landscape keeps the options fresh, so the References consent panel below folds that in (the web option runs a current landscape check before the stack questions).
